...
import com.lbdev.budgetbuzz.databinding.FragmentIncomeBinding
...
class IncomeFragment : Fragment(), CardItemAdaptor.OnItemClickListener, ImageLoaderFactory {
    ...
    override fun onCreateView(
        ...
        val recyclerView = view.findViewById<RecyclerView>(R.id.recyclerViewIncome)

        val startIndex = arguments?.getInt("startIndex", 0) ?: 0
        val count = arguments?.getInt("count", 9) ?: 9

        sharedViewModel.incomeCategories.observe(viewLifecycleOwner) { categories ->
            val itemsForPage =
                categories.subList(startIndex, min(startIndex + count, categories.size))
    ...
    companion object {
        fun newInstance(startIndex: Int, count: Int): IncomeFragment {
